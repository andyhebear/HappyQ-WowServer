...
{
    public partial class HomeControlSub1 : UserControl
    {
        public HomeControlSub1()
        {
            InitializeComponent();
        }
        ...
        private void HomeControlSub1_Load( object sender, EventArgs e )
        {
            if ( m_bisInit == true )
                return;
            else
            ...
            Column = axReportControl.Columns.Add( COLUMN_NAME, "报表名称", 250, true );
            Column.DrawHeaderRowsVGrid = false;

            Column = axReportControl.Columns.Add( COLUMN_FILE, "报表文件", 400, true );
            Column.DrawHeaderRowsVGrid = false;
        ...
        private void AddNewRecord()
        {
            ...
            if ( strFile != NEW_ITEM_FILE && String.IsNullOrEmpty( strFile ) == false )
            {
                ...
                {
                    AddTask( strName == NEW_ITEM_NAME ? string.Empty : strName, strFile );
