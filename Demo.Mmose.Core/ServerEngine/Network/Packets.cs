...
namespace Demo.Mmose.Core.Network
{
    ...
    public abstract class Packet
    {
        ...
        public Packet( long iPacketID, long iPacketLength, long iCapacity )
        {
            m_PacketID = iPacketID;
            m_PacketLength = iPacketLength;
            m_Stream = PacketWriter.Instance( iCapacity );  // 通过InternalCompile(...)的PacketWriter.ReleaseInstance(...)返回进入空闲的集合
        ...
        /// 输出包的输出流
        /// </summary>
        private PacketWriter m_Stream = null;
        #endregion
        /// <summary>
        /// 输出包的输出流
        /// </summary>
        ...
        {
            if ( m_Stream == null )
                throw new Exception( "Packet.AcquireCompile(...) - m_Stream == null error!" );
            ...
            if ( returnBuffer == null )
                throw new Exception( string.Format( "Packet.AcquireCompile(...) - returnBuffer == null | 0x{0:X2} ('{1}') (length={2})| error!", m_PacketID, GetType().Name, m_PacketLength ) );
